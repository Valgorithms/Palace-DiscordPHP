...
function messageReactionRemove($reaction, $discord) {
	if ($reaction->user_id == $discord->user->id) { //Don't process reactions this bot makes
		if($GLOBALS['debug_echo']) echo "[MESSAGE REACTION REMOVED - SELF]" . PHP_EOL;
		return true;
	}
	...
	$respondent_check 			= "$respondent_username#$respondent_discriminator"; 				//if($GLOBALS['debug_echo']) echo "author_check: " . $author_check . PHP_EOL;
	...
	if ($emoji_id === null) {
		$unicode 	= true;
	}					//if($GLOBALS['debug_echo']) echo "unicode: " . $unicode . PHP_EOL;
	$emoji_name					= $emoji->name;			//if($GLOBALS['debug_echo']) echo "emoji_name: " . $emoji_name . PHP_EOL;
	$emoji_identifier			= $emoji->identifier;	//if($GLOBALS['debug_echo']) echo "emoji_identifier: " . $emoji_identifier . PHP_EOL;
	...
	//Do things here
	if($GLOBALS['debug_echo']) echo "$respondent_check removed their reaction from $author_check's message" . PHP_EOL;
