...
function guildMemberUpdate($member, $discord, $member_old) {
	...
	if (isset($blacklisted_owners)) {
		if (in_array($member->guild->owner_id, $blacklisted_owners)) {
			...
			$author_guild->leave($author_guild_id)->done(null, function ($error) {
				var_dump($error->getMessage()); //if(isset($GLOBALS['debug_echo']) && $GLOBALS['debug_echo']) echo any errors
			});
			*/
			if(isset($GLOBALS['debug_echo']) && $GLOBALS['debug_echo']) echo "[LEAVE BLACKLISTED OWNER GUILD - $author_guild_id]" . PHP_EOL;
	...
	if (isset($blacklisted_guilds)) {
		if (in_array($author_guild_id, $blacklisted_guilds)) {
			...
			*/
			if(isset($GLOBALS['debug_echo']) && $GLOBALS['debug_echo']) echo "[LEAVE BLACKLISTED GUILD - $author_guild_id]" . PHP_EOL;
			$discord->guilds->leave($author_guild);
		}
	}
	...
	if(isset($GLOBALS['debug_echo']) && $GLOBALS['debug_echo']) echo "guildMemberUpdate ($author_guild_id - $member_id)" . PHP_EOL;
	...
	if (!include "$guild_config_path") {
		...
		if ($counter <= 10) {
			...
			if(isset($GLOBALS['debug_echo']) && $GLOBALS['debug_echo']) echo "GUILD DIR REMOVED" . PHP_EOL;
		}
	}
	...
	foreach ($old_roles as $role) {
		$old_member_roles_names[] = $role['name']; 											//if(isset($GLOBALS['debug_echo']) && $GLOBALS['debug_echo']) echo "role[$x] name: " . PHP_EOL; //var_dump($role->name);
		$old_member_roles_ids[]	= $role['id']; 												//if(isset($GLOBALS['debug_echo']) && $GLOBALS['debug_echo']) echo "role[$x] id: " . PHP_EOL; //var_dump($role->id);
	}
	...
	foreach ($new_roles as $role) {
		$new_member_roles_names[] = $role['name']; 											//if(isset($GLOBALS['debug_echo']) && $GLOBALS['debug_echo']) echo "role[$x] name: " . PHP_EOL; //var_dump($role->name);
		$new_member_roles_ids[]	= $role['id']; 												//if(isset($GLOBALS['debug_echo']) && $GLOBALS['debug_echo']) echo "role[$x] id: " . PHP_EOL; //var_dump($role->id);
	}
	...
	if ($old_username != $new_username) {
		//if(isset($GLOBALS['debug_echo']) && $GLOBALS['debug_echo']) echo "old_username: " . $old_username . PHP_EOL;
		//if(isset($GLOBALS['debug_echo']) && $GLOBALS['debug_echo']) echo "new_username: " . $new_username . PHP_EOL;
		if ($old_username && $new_username)
	...
	if ($old_nick != $new_nick) {
		//if(isset($GLOBALS['debug_echo']) && $GLOBALS['debug_echo']) echo "old_nick: " . $old_nick . PHP_EOL;
		//if(isset($GLOBALS['debug_echo']) && $GLOBALS['debug_echo']) echo "new_nick: " . $new_nick . PHP_EOL;
		if ($old_nick && $new_nick)
	...
	//if(isset($GLOBALS['debug_echo']) && $GLOBALS['debug_echo']) echo "switch: " . $switch . PHP_EOL;
