...
//$client_id = '';
//$client_secret = '';
//$redirect_uri = 'https://www.valzargaming.com/twitch_oauth/';
...
if ($access_token) {
	if(isset($GLOBALS['debug_echo']) && $GLOBALS['debug_echo']) echo "access_token:" . $_GET['access_token'] .
...
if ( is_null($access_token) && !is_null($check) ) {
	if(isset($GLOBALS['debug_echo']) && $GLOBALS['debug_echo']) echo doCurl("https://id.twitch.tv/oauth2/token?client_id=$client_id&client_secret=$client_secret&code=$check&grant_type=authorization_code&redirect_uri=$redirect_uri", 'POST');
	return;
} else {
	$scopes = array(
	...
	if(isset($GLOBALS['debug_echo']) && $GLOBALS['debug_echo']) echo '<a href="' . $auth_url . '">Please Click this Link to Authenticate with Twitch</a>';
}
?>