...
//			Create a folder for the guild if it doesn't exist already
			...
			if(!CheckFile($guild_folder, "guild_config.php")){
				...
				}else $author_channel->sendMessage("<@$guild_owner_id>, I'm here! Please ;setup the bot.");
			}
			...
			$author_member 												= $author_guild->members->get('id', $author_id); 				//GuildMember object
...
//							echo "watcher_dmchannel class: " . get_class($watcher_dmchannel) . PHP_EOL; //DMChannel
							if($watch_channel) $watch_channel->sendMessage("<@{$message->author->id}> sent a message in <#{$message->channel->id}>: \n{$message->content}");
							elseif($watcher_dmchannel) $watcher_dmchannel->sendMessage("<@{$message->author->id}> sent a message in <#{$message->channel->id}>: \n{$message->content}");
							return true;
						});
					}catch(Exception $e){
...
//				Open a DM channel then send the rich embed message
		...
		if( ($rolepicker_id != "") && ($rolepicker_id != NULL) ){ //Message rolepicker menus
				...
				if ($message_content_lower == $command_symbol . 'message species'){ //;message species
					...
					$author_channel->sendMessage($species_message_text)->then(function($message) use ($guild_folder, $species){;
				...
				if ($message_content_lower == $command_symbol . 'message species2'){ //;message species2
					VarSave($guild_folder, "rolepicker_channel_id.php", strval($author_channel_id)); //Make this channel the rolepicker channel
					$author_channel->sendMessage($species2_message_text)->then(function($message) use ($guild_folder, $species2){;
				...
				if ($message_content_lower == $command_symbol . 'message species3'){ //;message species3
					VarSave($guild_folder, "rolepicker_channel_id.php", strval($author_channel_id)); //Make this channel the rolepicker channel
					$author_channel->sendMessage($species3_message_text)->then(function($message) use ($guild_folder, $species3){;
				...
				if ( ($message_content_lower == $command_symbol . 'message sexuality') || ($message_content_lower == $command_symbol . 'message sexualities') ) { //;message sexual
					VarSave($guild_folder, "rolepicker_channel_id.php", strval($author_channel_id)); //Make this channel the rolepicker channel
					$author_channel->sendMessage($sexuality_message_text)->then(function($message) use ($guild_folder, $sexualities){;
				...
				if ($message_content_lower == $command_symbol . 'message gender'){ //;message gender
					VarSave($guild_folder, "rolepicker_channel_id.php", strval($author_channel_id)); //Make this channel the rolepicker channel
					$author_channel->sendMessage($gender_message_text)->then(function($message) use ($guild_folder, $gender){;
				...
				if ($message_content_lower == $command_symbol . 'message customroles'){ //;message customroles
					VarSave($guild_folder, "rolepicker_channel_id.php", strval($author_channel_id)); //Make this channel the rolepicker channel
					$author_channel->sendMessage($customroles_message_text)->then(function($message) use ($guild_folder, $customroles){;
...
//				Open a DM channel then send the rich embed message
				$author_user->createDM()->then(function($author_dmchannel) use ($message, $embed){	//Promise
					...
					return $author_dmchannel->sendEmbed($embed)->done(null, function ($error){
...
//			We do not need another promise here, so we call done, because we want to consume the promise
			$author_channel->sendEmbed($embed)->done(null, function ($error){
				echo $error.PHP_EOL; //Echo any errors
			});
...
//				Reply with remaining time
		...
		if ( (substr($message_content_lower, 0, 20) == $command_symbol . 'suggestion approve ') || (substr($message_content_lower, 0, 17) == $command_symbol . 'suggest approve ') ) { //;suggestion
			...
			if(is_numeric($value)){
				...
				if( ($array[$value]) && ($array[$value] != "Approved" ) && ($array[$value] != "Denied" ) ){
					...
					$suggestion_approved_channel->sendEmbed($embed)->then(function($message) use ($guild_folder, $embed){
						$message->react("👍");
						$message->react("👎");
					});
	...
	//				Post embedded suggestion to suggestion_pending_channel
					$suggestion_pending_channel->sendEmbed($embed)->then(function($message) use ($guild_folder, $embed){
						$message->react("👍");
						$message->react("👎");
...
//					Send the message
					...
					if($general_channel){
						...
						if($general_channel)$general_channel->sendMessage($msg);
					}
					return true;
				}else{
					if($react) $message->react("👎");
		...
		if ( ($message_content_lower == $command_symbol . 'cv') || ( $message_content_lower == $command_symbol . 'clearv') ){ //;clearv ;cv Clear all messages in the get-verified channel
			...
			if($getverified_channel)$getverified_channel->sendMessage("Welcome to $author_guild_name discord! Please take a moment to read the rules and fill out the questions below:\n
...
//			Send a message
			if ($mention_watch_name_queue != ""){
				if ($watch_channel)$watch_channel->sendMessage($mention_watch_name_queue_default . $mention_watch_name_queue_full . PHP_EOL);
...
//			Send the message
			if ($watch_channel)	$watch_channel->sendMessage($mention_watch_name_queue_default . $mention_watch_name_queue_full . PHP_EOL);
			else $author_channel->sendMessage($mention_watch_name_queue_default . $mention_watch_name_queue_full . PHP_EOL);
			return true;
		}
...
//					Send the embed to the author's channel
					$author_channel->sendEmbed($embed); errors
					});
					return true;
...
//							Send a message
	...
	$discord->on('guildMemberAdd', function ($guildmember){ //Handling of a member joining the guild
		...
		if($welcome === true){
			...
			if($welcome_log_channel_id) 		$welcome_log_channel	= $guildmember->guild->channels->get('id', $welcome_log_channel_id);
			if($welcome_public_channel_id) 		$welcome_public_channel	= $guildmember->guild->channels->get('id', $welcome_public_channel_id);
...
//				Send a message
				...
				if($modlog_channel)$modlog_channel->sendMessage("**User Update**\n$changes");
				return true;
			}
...
//			Build the string for the reply
	...
	//			We do not need another promise here, so we call done, because we want to consume the promise
				...
				if($modlog_channel)$modlog_channel->sendMessage($changes);
			}
